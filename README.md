...
普通使用者请直接阅读 [心影Pro 使用手册](docs/USER_GUIDE.md)。
...
- 项目创建、重命名、说明、删除，以及心影生成链接、历史对话选择与复用、模型、提示词、画幅、时长、分辨率和声音参数管理；Seedance 2.5 与 Seedance 2.0 是一等模型选项，参数与素材上限会按心影当前能力联动。
...
- 原片下载默认定位到系统“下载”目录，并以项目名、生成时间和批次序号命名；不再暴露内部任务 ID，也不会静默覆盖同名文件。
- 日间/夜间主题可一键切换并自动记忆。项目卡片、顶栏和首页在 1120px 最小窗口下有独立布局验收，不再出现状态文字压住按钮或人物横向溢出。
- 工作台按页面频率刷新并缓存心影登录状态；结果视频在悬停或打开时才加载，总览主视觉由 3.16 MB 优化到约 148 KB，减少冷启动与后台网络开销。
...
- APP 内置“Codex扩展”页面：把 `xinying-pro-generate` Skill 和指向当前安装版APP的本机 CLI 启动器安装到 Codex。无需源码或 Node.js；APP升级后可在同一页面更新 Skill。若已存在同名个人 Skill，确认后会先保留到 Codex 专用备份目录，不会静默覆盖，也不会让备份被 Codex 重复识别。
- Codex 默认只监控到本批次全部任务被心影接受并进入“生成中”，随即把自动提交视为完整成功并结束；结果完成、同步和下载由用户另行要求，不会让原生成对话长期卡在检查中。
...
心影Pro更新后，如果内置 Skill 有新版，“Codex扩展”页面会显示“可更新”。点击更新即可原子替换受管版本；未受管的同名 Skill 会先备份到 `$CODEX_HOME/skill-backups`。
...
进入“原网页模式”处理后，在任务队列点击“恢复”；CLI 使用 `job resume <id> --confirm`。详见 [使用手册](docs/USER_GUIDE.md)、[适配器维护说明](docs/ADAPTER_MAINTENANCE.md)、[安全说明](docs/SECURITY.md) 和 [交付验收记录](docs/VERIFICATION.md)。
