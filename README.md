...
5. 添加图片、视频或音频并拖动确认 APP 创作顺序；图片或含人物的视频可直接从卡片发起合规授权和审核，音频不会显示人像授权操作。
6. 如需复用已授权虚拟人像，在生成工作台下方点击角色；它会立即出现在上方参考素材中。图片角色使用 `@图N`，视频角色使用 `@视频N`，所有类型均可自由混排。提示词可写 `@图1`、`@图一` 或“参考图三”；APP 会在提交时统一识别并换算为心影的实际编号。
