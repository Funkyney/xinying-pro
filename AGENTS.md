...
- 查询命令可直接执行。
- `platform create`、`job submit`、`job cancel`、`portrait submit`、`portrait authorize-reference`、`director authorize`、`director submit` 和删除命令必须得到用户明确授权，并传入 `--confirm`。
...
9. 使用 `job status <job-id>` 和 `job events <job-id>` 查询进度。
