...
function PlatformProjectsPage({ snapshot, run, onOpened }: { snapshot: DashboardSnapshot; run: (action: () => Promise<unknown>, success?: string) => Promise<void>; onOpened: (project: Project) => void }) {
  ...
  return <div className="platform-projects-page">
    ...
    {conversationProject && <div className="modal-backdrop"><section className="confirm-modal conversation-picker-modal"><div className="modal-icon"><ListTree size={21} /></div><h2>选择“{conversationProject.name}”中的对话</h2><p>选择已有对话可继续复用上一条内容；精确的对话 ID 会保存到当前 APP 项目。</p><div className="conversation-list"><button className="conversation-row new" disabled={conversationsLoading} onClick={() => void openConversation()}><span className="conversation-icon"><Plus size={17} /></span><span><strong>新建对话</strong><small>从空白内容生成会话开始</small></span><ChevronRight size={15} /></button>{conversations.map((conversation) => <button className={`conversation-row ${conversation.isCurrent ? "current" : ""}`} key={conversation.id} onClick={() => void openConversation(conversation.id)}><span className="conversation-icon"><Clock3 size={16} /></span><span><strong>{conversation.title}</strong><small>{conversation.updatedAt ? `最近更新 ${formatDate(conversation.updatedAt)}` : `对话 ID：${conversation.id.slice(0, 12)}`}{conversation.isCurrent ? " · 心影当前" : ""}</small></span><ChevronRight size={15} /></button>)}{conversationsLoading && <div className="conversation-loading"><div className="loader" /><span>正在读取心影对话记录…</span></div>}{!conversationsLoading && !conversations.length && <div className="conversation-empty">当前项目还没有可复用的历史对话，可以新建一条。</div>}</div><div className="modal-actions"><button className="button ghost" onClick={() => setConversationProject(null)}>取消</button></div></section></div>}
  </div>;
}
