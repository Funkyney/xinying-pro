  ...
  return <div className="studio-page">
    ...
    <div className="studio-columns">
      ...
      <section className="panel parameters-panel">
        ...
        <div className="form-grid"><label>生成模式<select value={draft.mode} onChange={(event) => updateDraft("mode", event.target.value as ProjectInput["mode"])}>{(profile?.modes ?? ["text-to-video", "image-to-video", "reference-to-video", "first-last-frame"]).map((mode) => <option value={mode} key={mode}>{modeLabels[mode]}</option>)}</select></label><label>画面比例<select value={draft.aspectRatio} onChange={(event) => updateDraft("aspectRatio", event.target.value)}>{(profile?.aspectRatios ?? ["16:9", "9:16", "1:1", "4:3", "3:4", "21:9", "自适应"]).map((ratio) => <option key={ratio}>{ratio}</option>)}</select></label><label>时长<input type="number" min={profile?.minDuration ?? 4} max={profile?.maxDuration ?? 30} value={draft.duration} onChange={(event) => updateDraft("duration", Number(event.target.value))} /><small>{profile ? `${profile.minDuration}–${profile.maxDuration} 秒` : "4–30 秒"}</small></label><label>分辨率<select value={draft.resolution} onChange={(event) => updateDraft("resolution", event.target.value)}><option value="auto">跟随心影当前值（推荐）</option>{(profile?.resolutions ?? ["480p", "720p", "1080p", "1K", "2K", "4k"]).map((resolution) => <option value={resolution} key={resolution}>{resolutionLabel(resolution)}</option>)}</select></label></div><label className="switch-row"><div><strong>生成声音</strong><span>{profile?.audioSupported ? "当前心影模型支持音画同出" : "具体能力以当前心影页面为准"}</span></div><input type="checkbox" checked={draft.audioEnabled} onChange={(event) => updateDraft("audioEnabled", event.target.checked)} /></label>
      </section>
    </div>
