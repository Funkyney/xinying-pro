...
const IPC = {
  ...
  jobsRemoveMany: "jobs:remove-many",
...
const api: XinyingApi = {
  ...
  jobs: {
    ...
    refresh: (ids?: string[]) => ipcRenderer.invoke(IPC.jobsRefresh, ids),
    remove: (id: string) => ipcRenderer.invoke(IPC.jobsRemove, id),
    removeMany: (ids: string[]) => ipcRenderer.invoke(IPC.jobsRemoveMany, ids),
  ...
  results: {
    ...
    sync: (projectId: string, source: PlatformResultSource = "personal") => ipcRenderer.invoke(IPC.resultsSync, projectId, source),
